  ...
  while (pos < block.length()) {
    ...
    if (expr.startsWith(QStringLiteral("if "))) {
      ...
      if (elsePos != -1 && elsePos < ifEnd) {
        ...
        elseBlock = block.mid(elsePos + 7,
                              ifEnd - elsePos - 7); // 跳过 ${else}（7个字符）
      } else {
        // 没有 else 分支
 ...
 * - 字符串：直接返回
 */
QString TemplateEngine::evaluate(const QString &expr,
                                 const QJsonObject &context) const {
  // 处理循环变量 ${this} 或 ${.}
 ...
 * 结果：Hello
 */
QString TemplateEngine::processIf(const QString &condition,
                                  const QString &thenBlock,
                                  const QString &elseBlock,
                                  const QJsonObject &context) const {
  // 获取条件变量
 ...
 * - "(2 + 3) * 4" → 20（括号优先）
 */
double TemplateEngine::evalExpression(const QString &expr,
                                      const QJsonObject &context) const {
  int pos = 0; // 当前解析位置
 ...
 */
double TemplateEngine::evalAddSub(const QString &expr, int &pos,
                                  const QJsonObject &context) const {
  // 先解析左操作数（乘除法）
 ...
 * - "a * b / c" → 先计算 a，然后 * b，然后 / c
 */
double TemplateEngine::evalMulDiv(const QString &expr, int &pos,
                                  const QJsonObject &context) const {
  // 先解析左操作数（基本元素）
 ...
 */
double TemplateEngine::evalPrimary(const QString &expr, int &pos,
                                   const QJsonObject &context) const {
  // 跳过前导空白字符
  while (pos < expr.length() && expr[pos].isSpace()) {
    ++pos;
