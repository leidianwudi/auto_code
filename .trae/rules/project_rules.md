...
对同一个文件，`Edit` 工具一次只修改 **3-4 处**。一次性修改过多会导致 VS Code / Trae 插件卡死，后续修改全部失效。

- 超过 4 处修改：拆分到多次编辑中完成，编辑之间至少间隔一次 Read 确认文件状态。
- 有多处重复模式需要替换时：改用 `replace_all: true` 参数一次完成，或分批次编辑。