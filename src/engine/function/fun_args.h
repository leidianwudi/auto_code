 ...
 * 校验失败时设置 FunMgr 错误并返回 false，调用方直接 return QJsonValue()。
...
inline bool requireCount(const QJsonArray &args, int count, const QString &error) {
...
inline bool requireString(const QJsonArray &args, int index, const QString &error) {
...
inline bool requireObject(const QJsonArray &args, int index, const QString &error) {
...
inline bool requireNumber(const QJsonArray &args, int index, const QString &error) {
  if (index < args.size() && args.at(index).isDouble()) return true;
  FunMgr::setError(error);
  return false;
}

/// 第 index 个参数为数组，否则设置错误并返回 false
inline bool requireArray(const QJsonArray &args, int index, const QString &error) {
