 ...
 */
static void serializeJsonValue(const QJsonValue &v, QStringList &lines,
  ...
  case QJsonValue::Object: {
    ...
    for (int i = 0; i < keys.size(); ++i) {
      ...
      lines.append(prefix + QStringLiteral("  \"") + key +
                   QStringLiteral("\": "));
      serializeJsonValue(obj.value(key), lines, indent + 1,
                         i == keys.size() - 1);
    }
  ...
  case QJsonValue::Double:
    ...
    if (lines.last().contains(QLatin1Char('.'))) {
      while (lines.last().endsWith(QLatin1Char('0')))
        lines.last().chop(1);
      if (lines.last().endsWith(QLatin1Char('.')))
        lines.last().chop(1);
    }
    if (!isLast)
      lines.last() += QLatin1Char(',');
...
QString FormatCode::formatJson(const QString &input) {
  ...
  if (err.error != QJsonParseError::NoError)
    return input; // 解析失败，返回原始文本

  QStringList lines;
  lines.append(QString());
  serializeJsonValue(doc.isObject() ? QJsonValue(doc.object())
...
QString FormatCode::formatAc(const QString &input) {
  ...
  for (int i = 0; i < srcLines.size(); ++i) {
    ...
    int closeBraces = 0;
    int pos = 0;
    while (pos < trimmed.size() && (trimmed[pos] == QLatin1Char('}') ||
                                    trimmed[pos] == QLatin1Char(']'))) {
      ++closeBraces;
      ++pos;
    }
    ...
    if (closeBraces > 0)
      effectiveIndent = qMax(0, indent - closeBraces);
...
QString FormatCode::formatTpl(const QString &input) {
  ...
  static const QRegularExpression elseTagRe(QStringLiteral(R"(\$\{else\b)"));
  ...
  for (int i = 0; i < srcLines.size(); ++i) {
    ...
    if (closeTagRe.match(trimmed).hasMatch()) {
      // 检查是否以闭标签开头
      int firstClose = trimmed.indexOf(QStringLiteral("${/"));
      if (firstClose == 0) {
    ...
    if (openMatch.hasMatch()) {
      // 如果行以开标签开头，增加缩进
      int firstOpen = trimmed.indexOf(QStringLiteral("${"));
      // 只处理行首或紧跟在闭标签前的开标签
      if (firstOpen >= 0) {
        ++indent;
      }
